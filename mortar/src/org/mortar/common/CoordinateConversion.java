...
public class CoordinateConversion {
	...
	public MGRUTM latLon2MGRUTM(double latitude, double longitude) {
	...
	private class LatLon2UTM {
		public UTM convertLatLonToUTM(double latitude, double longitude) {
			...
			UTM utm = new UTM();
			utm.longZone = getLongZone(longitude);
			LatZones latZones = new LatZones();
			utm.latZone = latZones.getLatZone(latitude);

			utm.easting = (int) getEasting();
			utm.northing = (int) getNorthing(latitude);
		...
		protected void setVariables(double latitude, double longitude) {
			...
			A6 = (POW(p * sin1, 6) * nu * SIN(latitude) * POW(COS(latitude), 5) / 720)
					* (61 - 58 * POW(TAN(latitude), 2) + POW(TAN(latitude), 4) + 270 * e1sq * POW(COS(latitude), 2) - 330 * e1sq * POW(SIN(latitude), 2)) * k0
					* (1E+24);
	...
	public static class MGRUTM {
		public String longZone;
		public String latZone;
		public String digraph1;
		public String digraph2;
	...
	private class LatLon2MGRUTM extends LatLon2UTM {
		public MGRUTM convertLatLonToMGRUTM(double latitude, double longitude) {
			validate(latitude, longitude);
			...
			MGRUTM mgrs = new MGRUTM();
			mgrs.longZone = longZone;
			mgrs.latZone = latZone;
			mgrs.digraph1 = digraph1;
			mgrs.digraph2 = digraph2;
	...
	private class Digraphs {
		...
		private String[] digraph1Array = { "A", "B", "C", "D", "E", "F", "G", "H", "J", "K", "L", "M", "N", "P", "Q", "R", "S", "T", "U", "V", "W", "X", "Y",
				"Z" };
